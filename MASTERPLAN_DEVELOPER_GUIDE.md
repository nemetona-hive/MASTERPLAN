...
- The page links `vendor/fontawesome.subset.css`, generated by
  `scripts/build-icons.js` from `vendor/fontawesome.min.css`. Upstream stays in
  the repo as the source it reads; nothing links it, so it never ships. 30 icons
  and 1.4 KiB, where the full sheet was 1,895 icons and 72 KiB of render-blocking
  CSS — the largest blocking request on the page.
